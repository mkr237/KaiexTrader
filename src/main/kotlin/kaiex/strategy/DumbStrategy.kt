...
class DumbStrategy(val symbol: String,
    ...
    private var strategyPosition = 0f
    ...
    override fun onStart() {
        ...
        timer.scheduleAtFixedRate(object : TimerTask() {
            ...
            override fun run() {
                ...
                if(isLong) {
                    val size = maxLong - strategyPosition
                    log.info("BUYING $size")
                    createOrder(29000f, OrderSide.BUY, size)
                    strategyPosition += size // assumes we get filled at this price
                } else {
                    val size = abs(maxShort - strategyPosition)
                    log.info("SELLING $size")
                    createOrder(28000f, OrderSide.SELL, size)
                    strategyPosition -= size // assumes we get filled at this price
                }
